...
//2nd, go to edit data process at below
...
const {
  ...
  checkDuplicate,
  deleteContact,
  updateContacts, //8th, we require updateContacts
...
app.post(
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      //res.send(req.body); //3rd, we delete this as this is only for debugging
      //4th, we uncommented the code below:
      //addContact(req.body);//5th, we replace addContact to updateContact at below
      updateContacts(req.body); //plural as we want to update all contacts
      req.flash('msg', 'Contact Data has been updated!'); //6th, we replace added to update
      res.redirect('/contact');
      //7th, go to contacts.js to create update function
    }
  }
);
